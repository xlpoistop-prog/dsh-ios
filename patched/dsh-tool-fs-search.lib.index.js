...
let rgImplPromise;
/**
* The programmatic entry point of the packaged JavaScript ripgrep replacement,
* resolved lazily once per process.
*
* iOS cannot reliably exec the packaged `rg` launcher: it is a shebang script
* whose interpreter path does not survive the jbroot view split, so
* `ctx.subprocess.spawn` reports "ripgrep launch failed" before ripgrep ever
* runs. The implementation module next to the launcher is therefore imported
* in-process and called as a function — no exec, no shebang, no PATH, no
* sandbox. Its `runRgImpl` never calls `process.exit` and never swaps global
* streams, so one search cannot take the server down.
*
* @returns the implementation's `runRgImpl` function; the memoized promise
*   rejects when the package does not contain (or export) the implementation.
*/
function resolveRgImpl() {
	rgImplPromise ??= Promise.resolve().then(async () => {
		const rgPath = await resolveRgPath();
		const implPath = join(dirname(rgPath), "rg-impl.mjs");
		if (!existsSync(implPath)) throw new Error(`packaged ripgrep launcher has no in-process implementation next to ${rgPath}`);
		const implementation = await import(pathToFileURL(implPath).href);
		if (typeof implementation.runRgImpl !== "function") throw new Error(`packaged ripgrep implementation at ${implPath} does not export runRgImpl`);
		return implementation.runRgImpl;
	});
	return rgImplPromise;
}
/**
* Run the packaged JavaScript ripgrep implementation IN-PROCESS with a plain
* argv vector and return its complete raw stdout. The working directory is the
* calling agent's session cwd (`exec.agent.session.header.cwd`) when available,
* else `process.cwd()`. `exec.signal` is forwarded so the cooperative tool
* timeout (`@deepseek-ai/dsh-tool-call-timeout-policy`) and caller cancellation
* stop the in-process directory walk.
*
* iOS cannot reliably exec the packaged `rg` launcher — a shebang script whose
* interpreter path does not survive the jbroot view split — so this bypasses
* `ctx.subprocess.spawn` entirely and calls the implementation module directly
* via {@link resolveRgImpl}. `--no-config` is still prepended for parity (the
* implementation ignores it). The implementation enforces the raw-output cap
* itself: truncated stdout fails as `SEARCH_RAW_OUTPUT_OVERFLOW`.
*
...
* `SEARCH_RAW_OUTPUT_OVERFLOW`). A failure to resolve or import the
* implementation, or a throw from it, becomes `SEARCH_FAILED` with the original
* as `cause`; an abort already observed becomes `SEARCH_ABORTED` instead.
*
* @param ctx - retained for signature stability; the in-process implementation needs no subprocess service.
...
* @param graceMs - unused in the in-process form; retained for signature stability.
...
async function runRipgrep(ctx, exec, toolName, argv, rawOutputMaxBytes, graceMs, stderrMaxBytes) {
	...
	let outcome;
	try {
		outcome = await runImpl(["--no-config", ...argv], {
			cwd: workdir,
			signal: exec.signal,
		...
		if (exec.signal.aborted) throw new SearchError(`${toolName} was aborted before completion (tool timeout or caller cancellation)`, "SEARCH_ABORTED");
		throw new SearchError(`${toolName} search failed in its in-process ripgrep implementation`, "SEARCH_FAILED", { cause: error });
	}
	if (exec.signal.aborted || outcome.aborted) throw new SearchError(`${toolName} was aborted before completion (tool timeout or caller cancellation)`, "SEARCH_ABORTED");
	if (outcome.exitCode !== 0 && outcome.exitCode !== 1) throw classifyRunFailure(toolName, outcome.exitCode, outcome.stderr, outcome.stderrLossy);
	return {
		stdout: completeStdout(toolName, {
			text: outcome.stdout,
			lossy: outcome.stdoutLossy
		}, rawOutputMaxBytes),
