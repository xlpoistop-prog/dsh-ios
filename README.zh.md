...
**先确认一件事：你现在能从这台电脑 SSH 连上手机吗？**

* **能** —— 直接跑下面三条命令，脚本会自己往下走。
* **不能** —— 先看[安装](#安装)那一节（越狱、装 OpenSSH、装 PuTTY），
  连通了再回来。**脚本无法替你完成这些** —— 通往手机的 SSH 通道，末端是手机上的 `sshd`。
...
> 第一次不放心的话，在上面最后一行末尾加个 `--dry-run` 先看一眼 ——
> 它只打印打算做什么、**不改任何东西**。确认没问题后，**把 `--dry-run` 去掉**再跑一次。

**这几个参数是什么意思。** 脚本跑在**你的电脑上**，不在手机上 —— 它是通过 SSH 去操作**手机**：

| 参数 | 含义 |
|---|---|
| `git clone` / `cd` | 把仓库拉到你的电脑并进入目录。 |
...
| `--dry-run` | 只打印打算做什么，**不改任何东西**。可选，第一次跑建议加上。 |

**`--device` 里的主机填什么，取决于你的电脑怎么连到手机：**

| 你的连接方式 | 填什么 |
|---|---|
| **i4Tools 的「打开 SSH 通道」** | `mobile@127.0.0.1` —— 它把手机的 22 端口通过 USB 转发到**你电脑的** 22 端口。**前提是你电脑的 22 端口没被别的东西占着。** |
...
> **`127.0.0.1` 不是通用答案** —— 它只是 i4Tools 的默认值，因为那个工具恰好转发到**你电脑的** 22 端口。
